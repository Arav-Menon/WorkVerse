...
import { fetchOrganizationById, fetchAllOrganizations, fetchOrgWorkspaces, type FetchOrganization, type OrgWorkspace } from "@/lib/api/org.api";
  ...
  useEffect(() => {
    ...
    async function loadOrg() {
      try {
        setLoading(true);
        setError(null);
        const [data, orgsData, wsData] = await Promise.all([
                ...
                {workspaces.length === 0 ? (
                  ...
                  <div className="flex flex-col gap-2">
                    {workspaces.map((ws) => (
                      <button
                        key={ws.id}
                        onClick={() => router.push(`/workspace/${orgId}/${ws.id}`)}
                        className="flex items-center justify-between p-3 rounded-lg border border-zinc-800/50 hover:border-zinc-700 hover:bg-zinc-900/30 transition-all cursor-pointer text-left"
                      >
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-lg bg-zinc-900 border border-zinc-800 flex items-center justify-center text-zinc-400 text-xs font-semibold">
                            {ws.name.substring(0, 2).toUpperCase()}
                          </div>
                          <div>
                            <p className="text-xs font-medium text-white">{ws.name}</p>
                            <p className="text-[10px] text-zinc-500">{ws.spaceCount} spaces</p>
                          </div>
                        </div>
                        <i className="ti ti-chevron-right text-zinc-600 text-xs"></i>
                      </button>
                    ))}
                  </div>
                )}
