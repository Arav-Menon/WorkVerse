...
COPY --from=prerelease /usr/src/app/apps/cortex ./apps/cortex
...
COPY --from=prerelease /usr/src/app/packages/rbac ./packages/rbac
...
ENTRYPOINT [ "bun", "run", "--cwd", "apps/cortex", "start:cortex" ]
